...
2.  **Tier 2/3 (Agent):** Complex reasoning models (Llama3-Groq-Tool-Use) via `crush` (CLI tool by CharmBracelet) which supports MCP (Filesystem, Search).

## Goal
I need a bash script called `clood-ask` that mimics the experience of `claude-cli`. It should act as a smart router that transparently switches between a fast pipe-based tool (`mods`) and a heavy agentic tool (`crush`) depending on the complexity of the user's request.
...
- **If COMPLEX:** Launch `crush` in interactive mode.
...
- **For `crush`:** Pass it as the initial prompt context so the agent knows "I am working in the `auth-service` repository" immediately.

### 4. Interface Unification
- Support piping: `cat error.log | clood-ask "fix this"` (Should default to `mods` unless specified otherwise).
- Support arguments: `clood-ask "Refactor main.py"` (Should trigger `crush`).
- Support REPL: `clood-ask` (No args) should launch `crush` directly.

## Output
Please write the complete `clood-ask` bash script. Ensure it handles:
- Dependency checks (is `mods` and `crush` installed?).
